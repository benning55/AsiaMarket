<template>
  <div>
        ...
        <div class="vertical-center">
          <div class="shadow-md">
            <div @click="cartDrawer = !cartDrawer"
              ...
              <div
                  class=" text-white rounded-full h-5 w-5 flex items-center justify-center bg-green absolute count-position">
    ...
    <el-dialog
        :title="$t('token_exp')"
        :visible.sync="isTokenError && $route.name !== 'login'"
        center
        :show-close="false"
        :close-on-click-modal="false">
      <span>{{ $t('must_login_again') }}</span>
      <span slot="footer" class="dialog-footer">
    <el-button @click="goLogin" type="primary">Go to Login Again</el-button>
  </span>
    </el-dialog>
  </div>
</template>
    ...
    updateCart() {
      axios.get(`${this.$store.state.endpoints.host}/api/products/cart/`, {
        ...
        if (e.response.status === 401) {
          ...
          this.$message.error(this.$t('error_Oops_') + e.response.status + ', at load item in cart');
        }
